...
class VuePluginListRetrieveTests(APITestCase):
    ...
    def test_filter_by_multiple_tags(self):
        ...
        tagged_plugin = create_vue_plugin(name="Vue Plugin2")
        tagged_plugin.tags.add('Utility', 'Authentication')

        url = '{}?tags=Filters,Authentication'.format(reverse('vue_plugins-list'))
        response = self.client.get(url)
